...
def choose_cutoff(y_true, probability):
    """학습 데이터의 교차검증 예측값으로 cutoff를 선택합니다."""
...
def save_test_evaluation(
    ...
    """테스트 데이터에 target이 있을 때만 성능평가 결과를 저장합니다."""
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "cutoff_analysis.png",
        dpi=300,
    )
    plt.close()
    ...
    predictions["Positive_Probability"] = np.round(
        probability,
        5,
    )
    ...
    # 5. 회귀계수와 모델 저장
    # --------------------------------------------------------
    # Gradient Boosting 자체 변수 중요도
    fitted_model = final_model.named_steps["model"]
    feature_names = (
        final_model.named_steps["imputer"]
        .get_feature_names_out(best_features)
    )
    ...
    # --------------------------------------------------------
    # 6. 테스트 target 유무에 따라 평가 또는 예측만 수행
    # --------------------------------------------------------
    summary = {
        "Model": MODEL_NAME,
        "Mode": (
