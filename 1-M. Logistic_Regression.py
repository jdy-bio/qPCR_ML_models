...
def save_test_evaluation(test, y_test, probability, metrics, selected_cutoff, best_features, final_model,):
    ...
    if len(np.unique(y_test)) == 2:
        ...
        ).to_excel(OUTPUT_DIR / "ROC_curve_points.xlsx", index=False,)
...
def main():
    ...
    for combination in feature_combinations(features):
        ...
        cv_metrics = calculate_metrics(y_train, cv_probability, DEFAULT_CUTOFF, )
    ...
    ranking.to_excel(OUTPUT_DIR / "marker_combination_ranking.xlsx", index=False, )
    ...
    cutoff_table, selected_cutoff, cutoff_rule = (choose_cutoff(y_train, best_cv_probability, ))
    cutoff_table.to_excel(OUTPUT_DIR / "cutoff_table.xlsx", index=False, )
    ...
    if SAMPLE_COLUMN not in predictions.columns:
        predictions.insert(0, SAMPLE_COLUMN, [f"TEST_{index + 1:03d}" for index in range(len(predictions))], )

    predictions["Positive_Probability"] = np.round(probability, 5, )
    predictions["Selected_Cutoff"] = selected_cutoff
    predictions["Prediction"] = prediction
    predictions["Prediction_Label"] = (predictions["Prediction"].map({NEGATIVE_LABEL: "Negative", POSITIVE_LABEL: "Positive", }))
    ...
    coefficient_df.to_excel(OUTPUT_DIR / "logistic_coefficients_odds_ratio.xlsx",index=False, )
    ...
    pd.DataFrame([summary]).to_excel(OUTPUT_DIR / "analysis_summary.xlsx", index=False, )
