...
OUTPUT_DIR = SCRIPT_DIR / f"results_Logistic_Regression"
...
def save_confusion_matrix(cm):
    ...
    plt.ylabel("Actual")
    plt.title(f"Logistic_Regression Confusion Matrix")
    plt.tight_layout()
...
def main():
    ...
    plt.title(f"Logistic_Regression ROC Curve")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title(f"Logistic_Regression Precision-Recall Curve")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title(f"Logistic_Regression Calibration Curve")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("Cutoff")
    plt.ylabel("Metric")
    plt.title(f"Logistic_Regression Cutoff Analysis")
    plt.legend()
    plt.tight_layout()
    ...
    plt.ylabel("Marker")
    plt.title(f"Logistic_Regression Permutation Importance")
    plt.tight_layout()
    ...
    pd.DataFrame({
        ...
        "Odds_Ratio": np.exp(fitted.coef_[0]),
    }).sort_values(
        "Odds_Ratio",
        ascending=False
    ).to_excel(
        OUTPUT_DIR / "logistic_coefficients_odds_ratio.xlsx",
        index=False
    )
    ...
    print(f"Logistic_Regression 분석 완료")
